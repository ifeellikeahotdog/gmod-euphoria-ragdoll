-- UPGRADE: Joint System with Bounded Constraints
-- Fixes PID integral explosion and improves stability
...
-- NEW: Constants for safety
local MAX_ERROR_INTEGRAL = 50
local MAX_CONSTRAINT_FORCE = 5000
...
function Joint:New(bone1Index, bone2Index, jointType)
    ...
    self.ErrorIntegral = Vector(0, 0, 0)  -- UPGRADE: Use Vector instead of scalar
...
function Joint:CalculateConstraintForce(bone1, bone2, deltaTime)
    ...
    local errorDerivative = (error - self.LastError) / math.max(deltaTime, 0.001)
    
    -- UPGRADE: Accumulate and clamp error integral to prevent explosion
    ...
    if force:LengthSqr() > MAX_CONSTRAINT_FORCE^2 then
        force = force:GetNormalized() * MAX_CONSTRAINT_FORCE
    end
...
function Joint:UpdateRotationConstraint(bone1, bone2)
    ...
    if angleMag > self.RotationLimit then
        local correctionFactor = (angleMag - self.RotationLimit) / math.max(angleMag, 0.001)
...
function JOINT_SYSTEM:CreateSkeletonJoints(ragdoll)
    ...
    for _, jointDef in ipairs(joints) do
        ...
        if bone1Idx and bone2Idx and bone1Idx >= 0 and bone2Idx >= 0 then
