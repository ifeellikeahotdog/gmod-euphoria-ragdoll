-- UPGRADE: Enhanced Physics Engine
-- Implements proper bone transformation, frame-independent damping, and safer calculations
...
function RAGDOLL:New(ent)
    ...
    self.ImpactHistory = {}
    self.StrainedJoints = {}
    self.BoneTransformCache = {}  -- NEW: Cache for performance
...
function RAGDOLL:InitializeBones(boneCount)
    ...
    for i = 0, boneCount - 1 do
        ...
        if bone.Name and (string.find(bone.Name, "chest") or string.find(bone.Name, "torso")) then
            bone.Mass = EUPHORIA:GetConfig("RAGDOLL.TORSO_MASS")
        elseif bone.Name and string.find(bone.Name, "head") then
...
function RAGDOLL:UpdatePhysics(deltaTime)
    deltaTime = math.max(deltaTime or 0.01, 0.001)  -- Clamp minimum deltaTime
    
    -- UPGRADE: Match engine gravity
    ...
    for _, bone in ipairs(self.Bones) do
        ...
        -- UPGRADE: Frame-independent damping using exponential decay
        ...
        -- Clamp velocity to reasonable values to prevent instability
        local maxVelocity = 1000
        if bone.Velocity:LengthSqr() > maxVelocity^2 then
            bone.Velocity = bone.Velocity:GetNormalized() * maxVelocity
        end
...
-- NEW: Apply bone transforms to actual entity
function RAGDOLL:ApplyBoneTransforms()
    if not self.Entity or not self.Entity:IsValid() then return end
    
    local boneCount = self.Entity:GetBoneCount()
    
    for boneIdx, bone in ipairs(self.Bones) do
        if boneIdx < boneCount then
            local matrix = self.Entity:GetBoneMatrix(boneIdx)
            if matrix then
                -- Update position
                matrix:SetTranslation(bone.Position)
                
                -- Update rotation if we're tracking it
                if bone.Rotation then
                    local angMatrix = bone.Rotation:ToMatrix()
                    matrix:SetAngles(bone.Rotation)
                end
...
function RAGDOLL:ApplyDamping(dampingFactor)
    ...
    for _, bone in ipairs(self.Bones) do
        bone.Velocity = bone.Velocity * (1 - math.Clamp(dampingFactor, 0, 1))
    end
end
