-- BUGFIX: Ragdoll Base Class - Division by Zero Fix
-- Only changed lines with critical bugs
...
function RAGDOLL:UpdatePhysics(deltaTime)
    ...
    for _, bone in ipairs(self.Bones) do
        -- BUGFIX: Check mass > 0 before division
        if bone.Mass <= 0 then
            bone.Mass = 1  -- Default minimum mass
        end
